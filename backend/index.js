...
app.post("/api/articles", async (req, res) => {
  try {
    ...
    const articles = Array.isArray(input) ? input : [input]; // Wrap single article in array if needed

    const saved = await Article.insertMany(articles);
    res
      .status(201)
      .json({ message: `${saved.length} article(s) saved successfully.` });
  } catch (err) {
...
app.get("/api/articles", async (req, res) => {
  try {
    ...
    if (theme && articles.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: `No articles found for theme '${theme}'.` });
    }
...
app.delete("/api/articles", async (req, res) => {
  try {
    ...
    if (theme) {
      ...
      return res.json({
        message: `Deleted ${result.deletedCount} articles for theme '${theme}'.`,
      });
    }
    // No theme specified: delete all articles
    result = await Article.deleteMany({});
    return res.json({
      message: `Deleted ${result.deletedCount} articles from all themes.`,
    });
  } catch (err) {
